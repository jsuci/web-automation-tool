  ...
  const crawler = new PlaywrightCrawler({
    ...
    requestHandler: async ({ page, response, request, enqueueLinks }) => {
      ...
      if (useSitemaps === false) {
        await page.waitForSelector("#main-header a");
        const elementsFound = page.locator("#main-header a");
        console.log(
          `# of elements found in ${"#main-header a"}: `,
          (await elementsFound.all()).length
        );
        await enqueueLinks({
          strategy: crawlType,
